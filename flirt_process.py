...
class FLIRTProcess:
    def __init__(self, sigfile=None, binary=None) -> None:
        self.matches = {}
        self.sigfile = sigfile
        self.binary = binary

    def parse_elf(self, elffile):
        funcs = []
        segs = []
        secs = []
        ...
        try:
            with open(elffile, 'rb') as f:
                ...
                if not sec:
                    log.error("No symbol table found bin binary")
                ...
                for sec in elffile.iter_sections():
                    for segment in elffile.iter_segments():
                        if segment['p_type'] == 'PT_LOAD' and segment.section_in_segment(sec):
                            bsec = BinarySection()
                            bsec.addr = sec['sh_addr']
                            bsec.offset = sec['sh_offset']
                            bsec.size = sec['sh_size']
                            bsec.name = sec.name
                            secs.append(bsec)

        except ELFError:
            log.exception('Unable to parse ELF file')
        return imgbase, segs, funcs, secs

    def parse_binary_file(self, binfile):
        fd = open(binfile, 'rb')
        raw = fd.read()
        fd.close()
        if raw[:4] == b'\x7f\x45\x4c\x46':
            imgbase, segs, funcs, secs = self.parse_elf(binfile)
            format = 'ELF'
        else:
            log.error(
                'ELF or PE not recognized. The file will be analyzed as a RAW binary object...')
            imgbase, segs, funcs, secs = None, None, None, None
            format = 'RAW'
        return raw, format, imgbase, segs, funcs, secs

    def parse_signature_file(self, file):
        def next_byte(buf):
            ...
            while True:
                ...
                if cur_byte < 0x20:
                    ...
                    if (cur_byte & FUNCTION_D) or (cur_byte & FUNCTION_Q):
                        log.warning("investig. flag of pub name...")
                    cur_byte = next_byte(buf)
        ...
        def parse_node_variant_mask(node, buf):
            if node.len < 0x10:
                ...
                node.var_mask = (next_multibytes(buf) << 32) + \
                    next_multibytes(buf)
        ...
        def parse_node_bytes(node, buf):
            ...
            for i in range(node.len):
                node.var_bool_arr.append(
                    True if (node.var_mask & cur_mask_bit) else False)
        ...
        if header.magic != b"IDASGN":
            log.error('Not a FLIRT signature')
            return None, None
        ...
        header.lib_name_len = next_byte(buf)
    ...
    def identify_functions(self, node, buf):
        def crc16(buf, len):
            poly = 0x8408
            reg = 0xffff
            i = 0
            ...
            return int(crc.encode("latin1").hex(), 16)

        # if the current node has children
        if len(node.childs):
            for child in node.childs:
                self.identify_functions(child, buf)
        # if the current node has modules
            ...
            for i in range(len(pattern)):
                if variant[i]:
                    ...
                    # print(pattern[i],  end=" ")
                    re_pat += re.escape(chr(pattern[i]).encode('latin1'))
            # print("")
            # compile the regex
            # print(re_pat.hex())
            ...
            for match in matchs:
                ...
                for module in node.modules:
                    ...
                    bufcrc16 = crc16(
                        buf[match.start()+32:match.start()+32+module.crc_len], module.crc_len)
                            ...
                            if buf[match.start() + 32 + module.crc_len + tail_byte.offset] == tail_byte.value:
                                ...
                                for ffcn in module.pub_fcns:
                                    ...
                                    if addr in self.matches:
                                        self.matches[addr].append(ffcn)
                                    else:
                                        self.matches[addr] = [ffcn]
                                break
    ...
    def dump_header(self, header):
        ...
        print ("lib name len: %s"%header.lib_name_len)
    ...
    def run(self):
        ...
        if os.path.isfile(self.sigfile):
            sigfiles.append(self.sigfile)
        elif os.path.isdir(self.sigfile):
            sigfiles.extend([os.path.join(self.sigfile, fn)
                            for fn in next(os.walk(self.sigfile))[2]])
        # read the binary file
        buf, format, imgbase, segs, funcs, secs = self.parse_binary_file(
            self.binary)
        flag = 0.0
        res = None
        for sigf in sigfiles:
            ...
            root_node, header = self.parse_signature_file(sigf)
            # self.dump_header(header)
            # self.dump_node(root_node)
            if root_node is None or header is None:
                continue
            # do the job
            self.identify_functions(root_node, buf)
            log.debug("%s %d/%d (%s%%)" % (sigf, len(self.matches), header.num_fcns,
                      "{:.2f}".format(100 * float(len(self.matches))/float(header.num_fcns))))
            symbols = {}
            for addr in self.matches:
                if len(self.matches[addr]) > 1:
                    continue
                ...
                if format == 'ELF':
                    ...
                    for seg in segs:
                        if seg.addr + int(addr, 16) < seg.addr + seg.size:
                            found = True
                            func = self.matches[addr][0]
                            symbols[seg.addr + int(addr, 16)] = func.name
                            log.debug("0x%08x %s %x" % (
                                seg.addr + int(addr, 16), func.name, func.neg_offset))
                            break
                        if not found:
                            log.error("0x%x: %s not within a segment" % (
                                imgbase + int(addr, 16), self.matches[addr][0].name))
            if flag < float(len(self.matches))/float(header.num_fcns):
                flag = float(len(self.matches))/float(header.num_fcns)
                res = symbols
        return res
